$(function () {
    ...
    var homeScroll = function () {
        if ($(window).scrollTop() > 300) {
            GetHomeBottomPart();
            $(window).unbind('scroll', homeScroll);
        }
    };

    function GetHomeBottomPart() {
        $("main.home").addClass("loading");
        $.ajax({
            url: baseurl +"Home/HomeBottom",
            data: { articles: displayedArticles },
            type: "post",
            success: function (response) {
                $("main.home").append(response);
                $("main.home").removeClass("loading");
                $(".page-loader").remove();
                InitSwiperBlog();
                InitSwiperVideo();
                InitSwiperLeader();
                ...
                var bottomAd = $('#HomeBottomAd').html();
                $(bottomAd).appendTo('.home-bottom-ad-holder');
            }
        })
    }

    if ($('main').hasClass('home')) {
        //$(window).bind('scroll', homeScroll);
    }
  ...
  function scroll() {
    ...
    if (windowWidth > 767) {
      if ($(window).scrollTop() >= 100) {
        ...
        if (isClone === false) {
          $('.holder-menu').clone().append().insertAfter('.navbar-nav');
          isClone = true;
        }
      } else {
        $('nav').removeClass('fixed-header');
        $('.navbar .holder-menu').remove();
        isClone = false;
      }
    } else {
      if ($(window).scrollTop() >= 100) {
...
function InitSwiperPopup() {
  ...
  $('.js-img-btn, .gallery-thumb-wide').on('click', function () {
